...
class User {
    ...
    public function add($conn)
    {
        ...
        $sqlRequest =
            "INSERT INTO users(email, password, surname, name, gender, pathToImg, roleID)
             VALUES ('$this->email', '$this->password','$this->surname', '$this->name', '$this->gender','$pathToImg', '$this->roleID')";
    ...
    private static function deleteImageByID ($conn, $id)
    {
        ...
        echo "<br>Result: " . $result['pathToImg'] . "<br>";

        if (trim($result['pathToImg']) !== "") {
            $target_dir = "\public\uploads\\";
            $dir = dirname(__DIR__, 2);
            $target_dir = $dir . $target_dir;
            $target_file = $target_dir . $result['pathToImg'];
            echo "<br>Myphoto: " . $result['pathToImg'] . "<br>";
